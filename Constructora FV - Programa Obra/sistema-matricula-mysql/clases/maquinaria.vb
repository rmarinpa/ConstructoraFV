...
Public Class Maquinaria
    ...
    Public Function listar(ByVal fecha_in As String, ByVal fecha_fin As String, ByVal id_obra As Integer) As DataTable

        nue_var.dt = New DataTable
        nue_conexion.conectar()
        nue_var.da = New MySqlDataAdapter("SELECT distinct(rut), razon_social FROM consulta_arrendador where id_obra = " & id_obra & " and Fecha >= '" & fecha_in & "' and Fecha <= '" & fecha_fin & "' order by razon_social asc", nue_conexion.conex())
    ...
    Public Function listar2(ByVal fecha_in As String, ByVal fecha_fin As String) As DataTable
        nue_var.dt = New DataTable
        nue_conexion.conectar()
        nue_var.da = New MySqlDataAdapter("SELECT distinct(id_obra), nombre_obra FROM mov_tierra_v2 Where Fecha >= '" & fecha_in & "' and Fecha <= '" & fecha_fin & "' order by nombre_obra asc", nue_conexion.conex())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar()
        listar2 = nue_var.dt
        Return listar2
    ...
    Public Sub modificar(ByVal Cod_viajes As Integer, ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal ida_vuelta As String, ByVal kilometraje As Double, ByVal precio_m3_km As Double, ByVal cuenta_costo As String)
