...
Public Class Sincroniza11
    ...
    Public Function listar(ByVal id_obra As Integer) As DataTable
        ...
        nue_var.da = New MySqlDataAdapter("SELECT * FROM subcontratista where sincroniza = 1  and id_obra = " & id_obra & "", nue_conexion.conex())
    ...
    Public Function listar_remoto(ByVal id_obra As Integer) As DataTable
        ...
        nue_var.da = New MySqlDataAdapter("SELECT * FROM subcontratista where sincroniza <> 0  and id_obra = " & id_obra & "", nue_conexion.conex_remoto())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar_remoto()
        listar_remoto = nue_var.dt
        Return listar_remoto
    End Function
    Public Sub insertar(ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal rut As String, ByVal domicilio As String, ByVal razon_social As String, ByVal representante As String, ByVal representante_rut As String, ByVal comuna As String, ByVal ciudad As String)
        ...
        Try
            ...
            nue_conexion.desconectar_remoto()
            nue_var.consulta = "SELECT MAX(Id_subcontratista) from subcontratista"
            Try
                nue_conexion.conectar_remoto()
                nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex_remoto())
                nue_var.dr = nue_var.cmd.ExecuteReader()
                If nue_var.dr.Read Then
                    idd_sincroniza = nue_var.dr("MAX(Id_subcontratista)")
                End If
                nue_var.dr.Close()
                nue_conexion.desconectar_remoto()
        ...
        End Try
    End Sub
    Public Sub insertar_remoto(ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal rut As String, ByVal domicilio As String, ByVal razon_social As String, ByVal representante As String, ByVal representante_rut As String, ByVal comuna As String, ByVal ciudad As String)
        nue_var.consulta = "INSERT INTO subcontratista (sincroniza, version, nombre_obra, id_obra, rut, domicilio, razon_social, representante, representante_rut, comuna, ciudad) VALUES(?sincroniza, ?version, ?nombre_obra, ?id_obra, ?rut, ?domicilio, ?razon_social, ?representante, ?representante_rut, ?comuna, ?ciudad)"
        Try
    ...
    Public Sub modificar(ByVal Id_identificacion As Integer, ByVal sincroniza As Integer)
        nue_var.consulta = "UPDATE subcontratista SET sincroniza = ?sincroniza WHERE Id_subcontratista = ?Id_identificacion"
        Try
            nue_conexion.conectar()
    ...
    Public Function listar_modifica(ByVal id_obra As Integer) As DataTable
        nue_var.dt = New DataTable
        nue_conexion.conectar_remoto()
        nue_var.da = New MySqlDataAdapter("SELECT * FROM subcontratista where sincroniza <> 0  and sincroniza <> 1 and id_obra = " & id_obra & "", nue_conexion.conex_remoto())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar_remoto()
    ...
    End Function
    Public Sub modificar_tabla(ByVal Cod_UF As Integer, ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal rut As String, ByVal domicilio As String, ByVal razon_social As String, ByVal representante As String, ByVal representante_rut As String, ByVal comuna As String, ByVal ciudad As String)
        nue_var.consulta = "UPDATE subcontratista SET sincroniza = ?sincroniza, version = ?version, nombre_obra = ?nombre_obra, id_obra = ?id_obra, rut = ?rut, domicilio = ?domicilio, razon_social = ?razon_social, representante = ?representante, representante_rut = ?representante_rut, comuna = ?comuna, ciudad = ?ciudad  WHERE Id_subcontratista = ?Cod_subcontratista"
        Try
            nue_conexion.conectar()
            nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex())
            nue_var.cmd.Parameters.Add("?Cod_subcontratista", MySql.Data.MySqlClient.MySqlDbType.Int32).Value = Cod_UF
            nue_var.cmd.Parameters.Add("?sincroniza", MySql.Data.MySqlClient.MySqlDbType.Int32).Value = sincroniza
            nue_var.cmd.Parameters.Add("?version", MySql.Data.MySqlClient.MySqlDbType.Int32).Value = version
            nue_var.cmd.Parameters.Add("?nombre_obra", MySql.Data.MySqlClient.MySqlDbType.VarChar).Value = nombre_obra
            nue_var.cmd.Parameters.Add("?id_obra", MySql.Data.MySqlClient.MySqlDbType.Int32).Value = id_obra
