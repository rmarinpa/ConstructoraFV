...
Public Class Frm_viajes
    ...
    Sub actualizar_dgv()
        dgv_viajes.DataSource = nue_report.listar(id_obra)
    ...
    Private Sub btn_guardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_guardar.Click
        ...
        If (txt_ida_vuelta.Text <> "" And IsNumeric(txt_km.Text) And IsNumeric(txt_m3_km.Text) And rdbtn_m3_km.Checked = True) Or (txt_ida_vuelta.Text <> "" And IsNumeric(txt_m3_km.Text) And rdbtn_viaje.Checked = True) Then
            ...
            If sw = 0 Then
                nue_report.insertar(1, 0, Nombre_Obra, id_obra, ida_vuelta, kilometraje, precio_m3_km, cuenta_costo)
                actualizar_dgv()
            ElseIf sincroniza_local = 1 And sincroniza = 1 And sw <> 0 Then
                nue_report.modificar(CInt(Cod_viajes), 1, 0, Nombre_Obra, id_obra, ida_vuelta, kilometraje, precio_m3_km, cuenta_costo)
                actualizar_dgv()
            ElseIf sincroniza = 0 And sw <> 0 Then
                version_mod1 = version_mod1 + 1
                nue_report.modificar(CInt(Cod_viajes), 2, version_mod1, Nombre_Obra, id_obra, ida_vuelta, kilometraje, precio_m3_km, cuenta_costo)
                actualizar_dgv()
            Else
                version_mod1 = version_mod1 + 1
                nue_report.modificar(CInt(Cod_viajes), 2, version_mod1, Nombre_Obra, id_obra, ida_vuelta, kilometraje, precio_m3_km, cuenta_costo)
    ...
    Private Sub rdbtn_viaje_CheckedChanged(sender As Object, e As EventArgs) Handles rdbtn_viaje.CheckedChanged
