...
Public Class frm_menu_principal_IV_V2
    ...
    Private Sub btn_guardar1_Click(sender As Object, e As EventArgs) Handles btn_guardar1.Click
        ...
        If dgv_datosAgregados1.RowCount > 0 Then
            If (txt_1_rut.Text <> "" And txt_1_razon_social.Text <> "" And txt_1_servicio.Text <> "" And dgv_datosAgregados1.Rows(0).Cells(1).Value.ToString() <> "" And dgv_datosAgregados1.Rows(0).Cells(2).Value.ToString() <> "" And dgv_datosAgregados1.Rows(0).Cells(3).Value.ToString() <> "" And dgv_datosAgregados1.Rows(0).Cells(4).Value.ToString() <> "" And dgv_datosAgregados1.Rows(0).Cells(6).Value.ToString() <> "") Then
                sincroniza_local = 1
                ...
                If dgv_datosAgregados1.Rows.Count > 0 Then
                    For i = 0 To dgv_datosAgregados1.Rows.Count - 1
                        ...
                        ElseIf dgv_datosAgregados1.Rows(i).Cells(0).Value.ToString() = 0 Then
                            sw1 = 0
                        End If
                        ...
                        ElseIf sincroniza_local = 1 And sincroniza = 1 And sw1 <> 0 Then
                            nue_obra1.modificar_posicion(CInt(Cod_1), 1, 0, Nombre_Obra, id_obra, rut, servicio, item, unidad_pago, precio_un, cantidad, precio_total, obra, cuenta_contable, plazo_dias, valor_contrato, valor_pesos, valor_seguro)
    ...
    Sub actualizar_dgv1()
        ...
        'dgv_1_subcontratos.Columns(13).Visible = False
    ...
    Private Sub txt_1_razon_social_TextChanged(sender As Object, e As EventArgs) Handles txt_1_razon_social.TextChanged
        If Len(txt_1_razon_social.Text) > 0 Then
            ...
            If dgv_arrendador.Rows.Count > 1 Then
                ...
                'txt_1_servicio.DataSource = nue_obra1.listar8(id_obra, txt_1_rut.Text)
                'txt_1_servicio.DisplayMember = "servicio"
                'txt_1_servicio.ValueMember = "id_obra"
            ...
            Else
                ...
                'txt_1_servicio.DataSource = nue_obra1.listar8(id_obra, txt_1_rut.Text)
                'txt_1_servicio.DisplayMember = "servicio"
                'txt_1_servicio.ValueMember = "obra"
    ...
    Private Sub dgv_1_maquinas_MouseClick(sender As Object, e As MouseEventArgs) Handles dgv_1_subcontratos.MouseClick
        dgv_1_items.DataSource = nue_obra1.listar2_items(id_obra, dgv_1_subcontratos.CurrentRow.Cells(0).Value.ToString(), dgv_1_subcontratos.CurrentRow.Cells(2).Value.ToString())
        ...
        Cod_1 = dgv_1_items.Rows(0).Cells(6).Value.ToString()
        version_mod1 = CInt(dgv_1_items.Rows(0).Cells(8).Value.ToString())

        If dgv_1_items.Rows.Count > 0 Then
            dgv_datosAgregados1.Rows.Add(dgv_1_items.RowCount)
        End If

        txt_1_razon_social.Text = dgv_1_items.Rows(0).Cells(1).Value.ToString()
        txt_1_servicio.Text = dgv_1_items.Rows(0).Cells(12).Value.ToString()
        txt_1_plazo_dias.Text = dgv_1_items.Rows(0).Cells(20).Value.ToString()
        txt_1_valor_contrato.Text = dgv_1_items.Rows(0).Cells(21).Value.ToString()
        txt_1_valor_seguro.Text = dgv_1_items.Rows(0).Cells(23).Value.ToString()
        cmb_1_obra.Text = dgv_1_items.Rows(0).Cells(18).Value.ToString()

        sincroniza_local = CInt(dgv_1_items.Rows(0).Cells(7).Value.ToString())
        sincroniza_mod1 = 2

        For i = 0 To dgv_1_items.Rows.Count - 1

            dgv_datosAgregados1.Rows(i).Cells(0).Value = dgv_1_items.Rows(i).Cells(6).Value.ToString()
    ...
    Public Sub actualiza_cuentas_maquinaria()
        cmb_cuenta_contable.DataSource = nue_obra1.listar6(id_obra)
    ...
    Private Sub btn_1_agregar_linea_Click(sender As Object, e As EventArgs) Handles btn_1_agregar_linea.Click
        ...
        dgv_datosAgregados1.Rows(dgv_datosAgregados1.RowCount - 1).Cells(0).Value = 0
    End Sub
