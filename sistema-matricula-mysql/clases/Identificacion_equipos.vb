...
Public Class Identificacion_equipos
    ...
    Public Function suma_maquinas(ByVal rut As String, ByVal id_obra As Integer) As String
        nue_var.consulta = "SELECT SUM(turno) FROM arrendador_equipos WHERE rut = ?rut and id_obra = ?id_obra and sincroniza <> 3"
        nue_conexion.conectar()
        nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex())
        nue_var.cmd.Parameters.Add("?rut", MySql.Data.MySqlClient.MySqlDbType.VarChar).Value = rut
        nue_var.cmd.Parameters.Add("?id_obra", MySql.Data.MySqlClient.MySqlDbType.Int32).Value = id_obra
        ...
        If nue_var.dr.Read Then
            If IsDBNull(nue_var.dr("SUM(turno)")) Then
                suma_maquinas = 0
            Else
                suma_maquinas = nue_var.dr("SUM(turno)")
            End If
        End If

        nue_var.dr.Close()
        nue_conexion.desconectar()
        Return suma_maquinas
    End Function
