...
Public Class Storage
    ...
    Public Function listar(ByVal id_obra As Integer) As DataTable
        ...
        Return listar
    End Function
    Public Sub insertar(ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal N_Guia As Double, ByVal Litros As Double, ByVal Fecha As Date, ByVal Obra As String, ByVal precio As Double)
        nue_var.consulta = "INSERT INTO Storage_v2 (sincroniza, version, nombre_obra, id_obra, N_Guia, Litros, Fecha, Obra, precio) VALUES(?sincroniza, ?version, ?nombre_obra, ?id_obra, ?N_Guia, ?Litros, ?Fecha, ?Obra, ?precio)"
        Try
            nue_conexion.conectar()
    ...
    Public Sub modificar(ByVal Cod_adicional As Integer, ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal N_Guia As Double, ByVal Litros As Double, ByVal Fecha As Date, ByVal Obra As String, ByVal precio As Double)
        nue_var.consulta = "UPDATE Storage_v2 SET sincroniza = ?sincroniza, version = ?version, nombre_obra = ?nombre_obra, id_obra = ?id_obra, N_Guia = ?N_Guia, Litros = ?Litros, Fecha = ?Fecha, Obra = ?Obra, precio = ?precio WHERE Id_storage = ?Cod_adicional"
        Try
            nue_conexion.conectar()
            nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex())
    ...
    Public Function listar3_guias(ByVal id_obra As Integer, ByVal N_guia As Integer) As DataTable
        nue_var.dt = New DataTable
        nue_conexion.conectar()
        nue_var.da = New MySqlDataAdapter("SELECT * FROM consulta_fact_comp where id_obra = " & id_obra & " and sincroniza <> 3 and (isnull(id_factura) or N_guia =  " & N_guia & ") order by N_Guia asc", nue_conexion.conex())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar()
