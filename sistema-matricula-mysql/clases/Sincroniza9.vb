...
Public Class Sincroniza9
    ...
    Public Function listar(ByVal id_obra As Integer, ByVal fecha As String) As DataTable
        ...
        nue_var.da = New MySqlDataAdapter("SELECT * FROM Storage_v2 where id_obra = " & id_obra & " and Fecha >= '" & fecha & "'", nue_conexion.conex())
    ...
    Public Function listar_remoto(ByVal id_obra As Integer, ByVal fecha As String) As DataTable
        ...
        nue_var.da = New MySqlDataAdapter("SELECT * FROM Storage_v2 where sincroniza <> 0  and id_obra = " & id_obra & " and Fecha >= '" & fecha & "'", nue_conexion.conex_remoto())
        nue_var.da.Fill(nue_var.dt)
    ...
    Public Sub insertar(ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal N_Guia As Double, ByVal Litros As Double, ByVal Fecha As Date, ByVal Obra As String)
        ...
        Try
            ...
            nue_conexion.desconectar_remoto()
            nue_var.consulta = "SELECT MAX(Id_storage) from Storage_v2"
            Try
                nue_conexion.conectar_remoto()
                nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex_remoto())
                nue_var.dr = nue_var.cmd.ExecuteReader()
                If nue_var.dr.Read Then
                    idd_sincroniza = nue_var.dr("MAX(Id_storage)")
                End If
                nue_var.dr.Close()
                nue_conexion.desconectar_remoto()
        ...
        End Try
    End Sub
    Public Sub insertar_remoto(ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal N_Guia As Double, ByVal Litros As Double, ByVal Fecha As Date, ByVal Obra As String)
        nue_var.consulta = "INSERT INTO Storage_v2 (sincroniza, version, nombre_obra, id_obra, N_Guia, Litros, Fecha, Obra) VALUES(?sincroniza, ?version, ?nombre_obra, ?id_obra, ?N_Guia, ?Litros, ?Fecha, ?Obra)"
        Try
    ...
    Public Sub modificar(ByVal Id_identificacion As Integer, ByVal sincroniza As Integer)
        nue_var.consulta = "UPDATE Storage_v2 SET sincroniza = ?sincroniza WHERE Id_storage = ?Id_identificacion"
        Try
            nue_conexion.conectar()
    ...
    Public Function listar_modifica(ByVal id_obra As Integer) As DataTable
        nue_var.dt = New DataTable
        nue_conexion.conectar_remoto()
        nue_var.da = New MySqlDataAdapter("SELECT * FROM Storage_v2 where sincroniza <> 0  and sincroniza <> 1 and id_obra = " & id_obra & "", nue_conexion.conex_remoto())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar_remoto()
    ...
    End Function
    Public Sub modificar_tabla(ByVal Cod_adicional As Integer, ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal N_Guia As Double, ByVal Litros As Double, ByVal Fecha As Date, ByVal Obra As String)
    ...
    Public Sub modificar_eliminar(ByVal Cod_identifica As Integer, ByVal sincroniza As Integer, ByVal version As Integer)
        nue_var.consulta = "UPDATE Storage_v2 SET sincroniza = ?sincroniza, version = ?version WHERE Id_storage = ?Cod_identifica"
        Try
            nue_conexion.conectar()
            nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex())
    ...
    Public Sub modificar_temporal2(ByVal Id_seccion As Integer, ByVal Id_secundario As Integer, ByVal Id_obra As Integer, ByVal version As Integer)
        ...
        End Try
    End Sub
    Public Function verificar_version2(ByVal Id_storage As Integer, ByVal Id_obra As Integer) As Integer
        nue_var.consulta = "SELECT version FROM Storage_v2 WHERE Id_storage = ?Id_storage and id_obra = ?Id_obra"
        nue_conexion.conectar_remoto()
        nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex_remoto())
    ...
    End Function
    Public Sub modificar_remoto2(ByVal Cod_adicional As Integer, ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal N_Guia As Double, ByVal Litros As Double, ByVal Fecha As Date, ByVal Obra As String)
        nue_var.consulta = "UPDATE Storage_v2 SET sincroniza = ?sincroniza, version = ?version, nombre_obra = ?nombre_obra, id_obra = ?id_obra, N_Guia = ?N_Guia, Litros = ?Litros, Fecha = ?Fecha, Obra = ?Obra WHERE Id_storage = ?Cod_adicional"
        Try
            nue_conexion.conectar_remoto()
            nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex_remoto())
