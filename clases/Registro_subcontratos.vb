...
Public Class Registro_subcontratos
    ...
    Public Function listar6(ByVal fecha As String, ByVal id_obra As Integer) As DataTable
        nue_var.dt = New DataTable
        nue_conexion.conectar()
        nue_var.da = New MySqlDataAdapter("SELECT * FROM fecha_avance_v2 where Fecha = '" & fecha & "' and id_obra = " & id_obra & "  order by razon_social, servicio, item asc", nue_conexion.conex())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar()
        listar6 = nue_var.dt
        Return listar6
    ...
    Public Function listar7(ByVal fecha1 As String, ByVal fecha2 As String, ByVal fecha3 As String, ByVal id_obra As Integer, ByVal rut As String, ByVal servicio As String) As DataTable
        nue_var.dt = New DataTable
        nue_conexion.conectar()
        nue_var.da = New MySqlDataAdapter("SELECT * FROM fecha_avance_v2 where (Fecha = '" & fecha1 & "' or Fecha = '" & fecha2 & "' or Fecha = '" & fecha3 & "') and id_obra = " & id_obra & " and rut like '" & rut & "' and servicio like '" & servicio & "' order by Fecha, razon_social, servicio, item asc", nue_conexion.conex())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar()
        listar7 = nue_var.dt
        Return listar7
