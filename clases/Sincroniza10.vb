...
Public Class Sincroniza10
    ...
    Public Function listar(ByVal id_obra As Integer, ByVal fecha As String) As DataTable
        ...
        nue_var.da = New MySqlDataAdapter("SELECT * FROM tabla_uf_v2 where id_obra = " & id_obra & " and Fecha_termino >= '" & fecha & "'", nue_conexion.conex())
    ...
    Public Function listar_remoto(ByVal id_obra As Integer, ByVal fecha As String) As DataTable
        ...
        nue_var.da = New MySqlDataAdapter("SELECT * FROM tabla_uf_v2 where sincroniza <> 0  and id_obra = " & id_obra & " and Fecha_termino >= '" & fecha & "'", nue_conexion.conex_remoto())
        nue_var.da.Fill(nue_var.dt)
    ...
    Public Sub insertar(ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal Fecha_termino As Date, ByVal UF As Double)
        ...
        Try
            ...
            nue_conexion.desconectar_remoto()
            nue_var.consulta = "SELECT MAX(Id_UF) from tabla_uf_v2"
            Try
                nue_conexion.conectar_remoto()
                nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex_remoto())
                nue_var.dr = nue_var.cmd.ExecuteReader()
                If nue_var.dr.Read Then
                    idd_sincroniza = nue_var.dr("MAX(Id_UF)")
                End If
                nue_var.dr.Close()
                nue_conexion.desconectar_remoto()
        ...
        End Try
    End Sub
    Public Sub insertar_remoto(ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal Fecha_termino As Date, ByVal UF As Double)
        nue_var.consulta = "INSERT INTO tabla_uf_v2 (sincroniza, version, nombre_obra, id_obra, Fecha_termino, UF) VALUES(?sincroniza, ?version, ?nombre_obra, ?id_obra, ?Fecha_termino, ?UF)"
        Try
    ...
    Public Sub modificar(ByVal Id_identificacion As Integer, ByVal sincroniza As Integer)
        nue_var.consulta = "UPDATE tabla_uf_v2 SET sincroniza = ?sincroniza WHERE Id_UF = ?Id_identificacion"
        Try
            nue_conexion.conectar()
    ...
    Public Function listar_modifica(ByVal id_obra As Integer) As DataTable
        nue_var.dt = New DataTable
        nue_conexion.conectar_remoto()
        nue_var.da = New MySqlDataAdapter("SELECT * FROM tabla_uf_v2 where sincroniza <> 0  and sincroniza <> 1 and id_obra = " & id_obra & "", nue_conexion.conex_remoto())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar_remoto()
    ...
    End Function
    Public Sub modificar_tabla(ByVal Cod_UF As Integer, ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal Fecha_termino As Date, ByVal UF As Double)
    ...
    Public Sub modificar_eliminar(ByVal Cod_identifica As Integer, ByVal sincroniza As Integer, ByVal version As Integer)
        nue_var.consulta = "UPDATE tabla_uf_v2 SET sincroniza = ?sincroniza, version = ?version WHERE Id_UF = ?Cod_identifica"
        Try
            nue_conexion.conectar()
            nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex())
    ...
    Public Sub modificar_temporal2(ByVal Id_seccion As Integer, ByVal Id_secundario As Integer, ByVal Id_obra As Integer, ByVal version As Integer)
        ...
        End Try
    End Sub
    Public Function verificar_version2(ByVal Id_UF As Integer, ByVal Id_obra As Integer) As Integer
        nue_var.consulta = "SELECT version FROM tabla_uf_v2 WHERE Id_UF = ?Id_UF and id_obra = ?Id_obra"
        nue_conexion.conectar_remoto()
        nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex_remoto())
        nue_var.cmd.Parameters.Add("?Id_UF", MySql.Data.MySqlClient.MySqlDbType.Int32).Value = Id_UF
        nue_var.cmd.Parameters.Add("?Id_obra", MySql.Data.MySqlClient.MySqlDbType.Int32).Value = Id_obra
        nue_var.dr = nue_var.cmd.ExecuteReader()
        If nue_var.dr.Read Then
    ...
    End Function
    Public Sub modificar_remoto2(ByVal Cod_UF As Integer, ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal Fecha_termino As Date, ByVal UF As Double)
        nue_var.consulta = "UPDATE tabla_uf_v2 SET sincroniza = ?sincroniza, version = ?version, nombre_obra = ?nombre_obra, id_obra = ?id_obra, Fecha_termino = ?Fecha_termino, UF = ?UF WHERE Id_UF = ?Cod_UF"
        Try
            nue_conexion.conectar_remoto()
            nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex_remoto())
