...
Public Class Identificacion_subcontratos
    ...
    Public Sub insertar_cabecera(ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal rut As String, ByVal domicilio As String, ByVal razon_social As String, ByVal representante As String, ByVal representante_rut As String, ByVal comuna As String, ByVal ciudad As String)
        ...
        End Try
    End Sub
    Public Sub insertar_posicion(ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal rut As String, ByVal servicio As String, ByVal item As String, ByVal unidad_pago As String, ByVal precio_un As Double, ByVal cantidad As Double, ByVal precio_total As Double, ByVal obra As String, ByVal cuenta_contable As String, ByVal plazo As Double, ByVal valor_contrato As Double, ByVal valor_pesos As Double, ByVal valor_seguro As Double)
        nue_var.consulta = "INSERT INTO subcontrato (sincroniza, version, nombre_obra, id_obra, rut, servicio, item, unidad_pago, precio_un, cantidad, precio_total, obra, cuenta_contable, plazo, valor_contrato, valor_pesos, valor_seguro) VALUES(?sincroniza, ?version, ?nombre_obra, ?id_obra, ?rut, ?servicio, ?item, ?unidad_pago, ?precio_un, ?cantidad, ?precio_total, ?obra, ?cuenta_contable, ?plazo, ?valor_contrato, ?valor_pesos, ?valor_seguro)"
        Try
            nue_conexion.conectar()
            nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex())
            nue_var.cmd.Parameters.Add("?sincroniza", MySql.Data.MySqlClient.MySqlDbType.Int32).Value = sincroniza
    ...
    End Sub
    Public Sub modificar_posicion(ByVal Cod_identifica As Integer, ByVal sincroniza As Integer, ByVal version As Integer, ByVal nombre_obra As String, ByVal id_obra As Integer, ByVal rut As String, ByVal servicio As String, ByVal item As String, ByVal unidad_pago As String, ByVal precio_un As Double, ByVal cantidad As Double, ByVal precio_total As Double, ByVal obra As String, ByVal cuenta_contable As String, ByVal plazo As Double, ByVal valor_contrato As Double, ByVal valor_pesos As Double, ByVal valor_seguro As Double)
        nue_var.consulta = "UPDATE subcontrato SET sincroniza = ?sincroniza, version = ?version, nombre_obra = ?nombre_obra, id_obra = ?id_obra,  rut = ?rut, servicio = ?servicio, item = ?item, unidad_pago = ?unidad_pago, precio_un = ?precio_un, cantidad = ?cantidad, precio_total = ?precio_total, obra = ?obra, cuenta_contable = ?cuenta_contable, plazo = ?plazo, valor_contrato = ?valor_contrato, valor_pesos = ?valor_pesos, valor_seguro = ?valor_seguro WHERE id_subcontrato = ?Cod_identifica"
        Try
            nue_conexion.conectar()
            nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex())
    ...
    Public Function listar2(ByVal id_obra As Integer) As DataTable
        ...
        nue_conexion.conectar()
        nue_var.da = New MySqlDataAdapter("SELECT rut, razon_social, servicio FROM subcontratista_subcon Where id_obra = " & id_obra & " and sincroniza <> 3 Group by rut, razon_social, servicio asc", nue_conexion.conex())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar()
        listar2 = nue_var.dt
        Return listar2
    End Function
    Public Function listar2_items(ByVal id_obra As Integer, ByVal rut As String, ByVal servicio As String) As DataTable

        nue_var.dt = New DataTable
        nue_conexion.conectar()
        nue_var.da = New MySqlDataAdapter("SELECT * FROM subcontratista_subcon Where id_obra = " & id_obra & " and rut like '" & rut & "' and servicio like '" & servicio & "' and sincroniza <> 3 order by id_subcontrato asc", nue_conexion.conex())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar()
